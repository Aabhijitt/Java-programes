...
public class chessboard {
	...
	public static void main(String[] args) {
		...
		printNQueensreactive(chess, 0, "", -1);
	...
	private static void printNQueensreactive(boolean[][] chess,int qno,String asf, int plqbno) {
       if (qno == chess.length) {
			count++;
			if(chessboardissafe(chess)==true){
			...
			for (int bno = plqbno + 1; bno < chess.length * chess.length; bno++) {
				...
				if (chess[row][col] == false) {
					chess[row][col] = true;
					printNQueensreactive(chess,qno + 1, asf + "q" + (qno + 1) + "b" + bno + " ",  bno);
					chess[row][col] = false;
				}
			}
	...
	for(int di = 0; di < dirs.length;di++){   //di - direction index.
		for(int dist = 1; true;dist++){ //dist --- distance..........true --- don't know the distance.as long as the board goes...
